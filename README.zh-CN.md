# Margin｜Mac 上的 Apple Books 语境阅读助手
...
Margin 是一个个人优先、开放源码的 macOS 英语 → 简体中文阅读助手。在 Apple
Books 中选择单词或短文并按一次快捷键，就能在书页旁查看结果，而不必切换到
通用翻译软件。
...
已验证环境是 macOS 26.5 和 Apple Books 8.5：选择文字，然后按
`Control–Option–M`（`⌃⌥M`）。该版本 Books 的阅读视图没有开放直接的
macOS“服务”传递；已支持的路径是明确按下快捷键，并使用用户批准的辅助功能
权限。准确边界见[兼容性记录](docs/compatibility-spike.md)。

本仓库**只发布源码**，不提供公开 `.app`、DMG、托管服务或普通用户安装支持。
凭据方案是个人 BYOK：你自行构建 Margin，并使用自己的服务商 API Key。

## Margin 的差异

- **在 Mac 上留在书页。** 一次快捷键只打开一个可复用浮层；关闭后立即回到
  阅读。
- **为书面文本设计。** 段落翻译以自然、可出版的中文为目标，重点覆盖小说、
  传记、历史和非虚构作品中常见的 2–4 句选区。
- **同一译文，两种视图。** “自然译文”优先显示完整中文；“双语对照”把同一份
  译文重组为带句号标记的英中对照块，不会生成相互矛盾的第二份译文。
...
Mac 版的优势是更安静的 Apple Books 阅读流程，以及专门针对自然中文书面表达
的翻译提示；它由 AI 生成的词典内容不是权威词典来源。

## 阅读体验

### 段落查询

段落结果默认进入“自然译文”，完整中文是主要内容，英文原文收在默认关闭的
“查看英文原文”中。结构化对齐可用时，可以切换到“双语对照”，按顺序阅读带
句号或句号范围的英中对照块。完整译文直接由这些有序中文片段拼接而成，因此
两种视图使用完全相同的中文措辞。只有结果包含至少两个对齐块时才会显示模式
切换；零个或一个对齐块会安静地停留在“自然译文”。

英文原文默认折叠，只有需要时才展开。短结果保持紧凑；长结果在 Mac 的
280–620 pt 浮层内滚动，复制、朗读、收藏和重试始终可用，并继续使用原生
Apple 图标。
...
你可以从 **Margin → 设置…**、按 `⌘,`，或点击菜单栏中的 Margin 图标打开同一个
设置窗口。
...
完整环境要求、无签名测试命令、签名配置、固定 Mac 安装与缓存清理见

...
- **只在需要时加一句说明。** 只有当歧义会改变含义、语气或指代对象时，才附一句简短
  提示——不是每次都给。
- **只发送必要内容。** Apple Books 可能随选区附加书名、作者等来源尾注；Margin 会先
  在本地清除，并且不会另行采集页码或选区周围文本。
...
Margin 按照数据最小化原则设计：发送给服务商的书籍内容只有在本地清理、规范化后的
选区；请求还包含完成翻译所需的固定指令与控制字段。Apple Books 可能附加的书名、
作者等来源尾注会先被删除，Margin 不会另行采集页码或选区周围文本。API Key 保存在
仅限本机的钥匙串项目中；结果进入可随时清除的小型本地缓存；只有主动点击收藏，内容
才会进入**已收藏**。选中文字仍会发送给你配置的服务商，因此 Margin 注重隐私，但
并非离线工具。详见 [SECURITY.md](SECURITY.md)。
